...
class BugContestState
  def pbJudge
    ...
    raise _INTL("No hay encuentros de Concurso de Bichos/Terreno para ningún mapa de Concurso de Bichos.") if maps_with_encounters.empty?
    @contestants.each do |cont|
      ...
      raise _INTL("No hay encuentros para el mapa {1}, así que no puedo juzgar el concurso.", enc_data[0]) if !enc
    ...
    if judgearray.length < 3
      raise _INTL("Hay muy pocos participantes para el Concurso de Bichos.")
    end
