    ...
    if pbPlayer.party_full? && (@sendToBoxes == 0 || @sendToBoxes == 2)   # Ask/must add to party
      ...
      loop do
        ...
        when 0   # Add to your party
          pbDisplay(_INTL("Elige un Pokémon de tu equipo para enviarlo a la Caja."))
          party_index = -1
          ...
          pbDisplayPaused(_INTL("{1} ha sido enviado a la Caja \"{2}\".", send_pkmn.name, box_name))
...
class Battle::Move::LowerPPOfTargetLastMoveBy3 < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("¡Los PP de {2} de {1} se han reducido en {3}!",
  ...
  def pbEORShiftDistantBattlers
    ...
    if !singleBattle?
      ...
      swaps.each do |pair|
        ...
        when 2
          pbDisplay(_INTL("¡{1} se ha movido!", @battlers[pair[1]].pbThis))
        when 3
          pbDisplay(_INTL("¡{1} se ha movido al centro!", @battlers[pair[1]].pbThis))
        end
      end
    end
  end
  ...
  def pbCanInflictStatus?(newStatus, user, showMessages, move = nil, ignoreStatus = false)
    ...
    if affectedByTerrain?
      ...
      when :Electric
        if newStatus == :SLEEP
          if showMessages
            @battle.pbDisplay(_INTL("¡{1} se ha rodeado de un campo eléctrico!", pbThis(true)))
          end
          return false
        end
      when :Misty
        @battle.pbDisplay(_INTL("¡{1} se ha envuelto en una espesa niebla!", pbThis(true))) if showMessages
        return false
      end
    end
    # Uproar immunity
    if newStatus == :SLEEP && !(hasActiveAbility?(:SOUNDPROOF) && !@battle.moldBreaker)
      @battle.allBattlers.each do |b|
        ...
        @battle.pbDisplay(_INTL("¡Pero el alboroto ha mantenido despierto a {1}!", pbThis(true))) if showMessages
        return false
      end
    end
    # Type immunities
    ...
    if immuneByAbility
      if showMessages
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          case newStatus
          when :SLEEP     then msg = _INTL("¡{1} sigue despierto!", pbThis)
          when :POISON    then msg = _INTL("¡{1} no puede ser envenenado!", pbThis)
          when :BURN      then msg = _INTL("¡{1} no puede sufrir quemaduras!", pbThis)
          ...
          case newStatus
          when :SLEEP
            msg = _INTL("¡{1} sigue despierto debido a {3} de {2}!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :POISON
            msg = _INTL("¡{1} no puede ser envenenado debido a {3} de {2}!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :BURN
            msg = _INTL("¡{1} no puede sufrir quemaduras debido a {3} de {2}!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :PARALYSIS
            msg = _INTL("¡{1} no puede ser paralizado debido a {3} de {2}!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :FROZEN
            msg = _INTL("¡{1} no puede ser congelado debido a {3} de {2}!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          end
        else
          case newStatus
          when :SLEEP     then msg = _INTL("¡{1} sigue despierto gracias a {2}!", pbThis, abilityName)
          when :POISON    then msg = _INTL("¡{2} de {1} evita que sea envenenado!", pbThis, abilityName)
          when :BURN      then msg = _INTL("¡{2} de {1} evita que sufra quemaduras!", pbThis, abilityName)
          when :PARALYSIS then msg = _INTL("¡{2} de {1} evita que sea paralizado!", pbThis, abilityName)
          when :FROZEN    then msg = _INTL("¡{2} de {1} evita que sea congelado!", abilityName)
          end
        end
        @battle.pbDisplay(msg)
...
class Battle::Battler
  def pbRecoverHPFromDrain(amt, target, msg = nil)
    if target.hasActiveAbility?(:LIQUIDOOZE, true)
      ...
      pbReduceHP(amt)
      @battle.pbDisplay(_INTL("¡{1} ha absorbido la Viscosecreción!", pbThis))
      @battle.pbHideAbilitySplash(target)
      pbItemHPHealCheck
    else
      msg = _INTL("¡{1} se ha quedado sin energía!", target.pbThis) if nil_or_empty?(msg)
      @battle.pbDisplay(msg)
      if canHeal?
...
class Battle::Move::HealUserByTargetAttackLowerTargetAttack1 < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    if target.hasActiveAbility?(:LIQUIDOOZE, true)
      ...
      @battle.pbDisplay(_INTL("¡{1} ha absorbido la Viscosecreción!", user.pbThis))
      @battle.pbHideAbilitySplash(target)
