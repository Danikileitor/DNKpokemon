...
MenuHandlers.add(:pc_menu, :pokemon_storage, {
  "name"      => proc {
    next ($player.seen_storage_creator) ? _INTL("{1}'s PC", pbGetStorageCreator) : _INTL("PC de Alguien")
  },
  "order"     => 10,
  "effect"    => proc { |menu|
    pbMessage(_INTL("\\se[PC access]Has accedido al Sistema de Almacenamiento Pokémon."))
    command = 0
    loop do
      command = pbShowCommandsWithHelp(nil,
         [_INTL("Organizar Cajas"),
          ...
          _INTL("Dejar Pokémon"),
          _INTL("Atrás")],
         [_INTL("Organiza los Pokémon de las Cajas y de tu equipo."),
          _INTL("Saca un Pokémon de las Cajas a tu equipo."),
          _INTL("Deposita un Pokémon de tu equipo en las Cajas."),
...
def pbChangeLevel(pkmn, new_level, scene)
  ...
  if pkmn.level == new_level
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("El nivel de {1} no ha variado.", pkmn.name))
    else
      pbMessage(_INTL("El nivel de {1} no ha variado.", pkmn.name))
    end
    return
  end
...
class PokemonEvolutionScene
  def pbEvolutionSuccess
    ...
    pbMessageDisplay(@sprites["msgwindow"],
                     _INTL("\\se[]¡Enhorabuena! ¡Tu {1} ha evolucionado a {2}!\\wt[80]",
    ...
    if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned && $player.has_pokedex
      pbMessageDisplay(@sprites["msgwindow"],
                       _INTL("Los datos de {1} se han registrado en la Pokédex.", newspeciesname)) { pbUpdate }
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if !@adapter.canSell?(item)
        pbDisplayPaused(_INTL("Lo siento, no puedo comprar {1}.", itemnameplural))
        next
      end
      ...
      if qty > 1
        qty = @scene.pbChooseNumber(
          _INTL("¿Que cantidad de {1} te gustaría vender?", itemnameplural), item, qty
        )
      end
      if qty == 0
        @scene.pbHideMoney
      ...
      price *= qty
      if pbConfirm(_INTL("Te puedo pagar {1}$.\n¿Te parece bien?", price.to_s_formatted))
        ...
        pbDisplayPaused(_INTL("Has obtenido {2}$ por {1}.",
